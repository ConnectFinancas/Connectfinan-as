    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KpiCard
          ...
          accent="#e8722c"
        />
        <KpiCard label="Entradas (12 meses)" value={totalEntradas} icon={ArrowUpCircle} accent="#0f9d6a" hint="acumulado" />
        <KpiCard label="Saídas (12 meses)" value={totalSaidas} icon={ArrowDownCircle} accent="#dc2626" hint="acumulado" />
      </div>

      <div className="card p-5">
        <h2 className="mb-1 text-sm font-semibold text-brand-900">Evolução do Saldo em Caixa</h2>
        <p className="mb-2 text-xs text-slate-400">Setembro/2025 a Agosto/2026 (mês corrente parcial)</p>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border-subtle bg-surface-muted text-left text-xs text-slate-500">
            ...
            <tbody>
              {cashFlow.map((c) => (
                <tr key={c.month} className="border-b border-border-subtle last:border-0">
                  ...
                  <td className="py-2.5 text-right tabular-nums text-accent-600">{formatCurrencyPrecise(c.entradas)}</td>
